...
// API links
const weatherApiKey = 'https://api.openweathermap.org/data/2.5/weather?q';  // Replace with your actual weather API key

// Function to fetch weather data
const getWeather = async (city) => {
  try {
    const res = await axios.get(`http://api.weatherapi.com/v1/current.json?key=${weatherApiKey}&q=${city}`);
    return res.data;
  } catch (error) {
...
// Create group command functions
cmd({
  pattern: "weather",
  alias: ["wth", "weatherinfo"],
  react: "🌤️",
  ...
  try {
    if (!q) return await reply("Please provide a city name!");
    
    const weatherData = await getWeather(q);
    const weatherMessage = `
      🌤️ *Weather Info for ${q}* 🌤️
    ...
    console.log(error);
    await reply("Sorry, I couldn't fetch the weather information.");
  }
});
